...
auto makeWorld(std::shared_ptr<ParametersTable> PT) -> std::shared_ptr<AbstractWorld> {
  ...
  if (worldType == "MODES") {
    newWorld = std::make_shared<MODESWorld>(PT);
    found = true;
    }
  if (!found){
...
void configureDefaultsAndDocumentation(){
  ...
  Parameters::root->setParameter("WORLD-worldType", (std::string)"MODES");
  Parameters::root->setDocumentation("WORLD-worldType","world to be used, [MODES]");
}
